...
export async function daoFindReimByStatus(status):Promise<reimbursement[]>
{
    ...
    try {
        ...
        (' select reim."reimbursementId", u.username as author,reim.amount,reim."dateSubmitted" ,reim."dateResolved" ,reim.description ,reim.resolver as resolver ,reim.status,reim.statusID ,reim."type" from reim  ,reimbursement.users u where  reim.author =u."userId" and reim.statusId = $1; ', [status])
       ...
       return results.rows.map(reimDTOReimConverter)
        //return results
        //return reimDTOReimConverter(results.rows[])
    } catch(e){
        console.log(e);
